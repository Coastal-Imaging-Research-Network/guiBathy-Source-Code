...
function [IOEO IOEOError]= IOEOSolver(IOEOInitialGuess,IOEOKnownsFlag,intrinsics,UV,xyz)
...
%  not be solved for and taken as constants in this solution. 
    ...
    [IOEO,R,J,CovB]   = nlinfit(xyz,[UV(:,1); UV(:,2)],@(IOEO,xyz)xyz2DistUVIOEO(intrinsics,IOEO,xyz), IOEOInitialGuess);
...
[eunknownSol,R,J,CovB]   = nlinfit(xyz,[UV(:,1); UV(:,2)],@(IOEOUnknown,xyz)xyz2DistUVforNLinFitIOEO(IOEOKnownsFlag,IOEOKnown,IOEOUnknown, intrinsics,xyz),IOEOUnknownInitialGuess);
...
function [UVd]= xyz2DistUVforNLinFitIOEO(IOEOKnownsFlag,IOEOKnown, IOEOUnknown, intrinsics,xyz)
...
iextrinsics=nan(1,7);
iextrinsics(kInd)=IOEOKnown;
iextrinsics(ukInd)=IOEOUnknown;


[UVd]= xyz2DistUVIOEO(intrinsics,iextrinsics,xyz);
end
